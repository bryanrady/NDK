...
template<typename T>
class SafeQueue {
    typedef void (*ReleaseCallback)(T *);   //这个函数指针接收一个指针，T是一个AVPacke *,所以参数传递一个指针的指针
    //typedef void (*ReleaseCallback2)(T &); //这个函数指针接收一个引用，T是一个AVPacke *,所以参数传递一个指针的引用
...
//    void setReleaseCallback2(ReleaseCallback2 r2) {
//        releaseCallback2 = r2;
//    }
...
private:
    ...
    //ReleaseCallback2 releaseCallback2;
